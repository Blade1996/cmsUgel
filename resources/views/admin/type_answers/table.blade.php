...
<table id="sectionsTable" class="table table-bordered table-striped">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>{{ $type_answer->name }}</td>
            <td>
                <img class="img-fluid" src="{{asset($type_answer->url_image)}}" width=200 height=100 alt="">
            </td>
            <td>{{ $type_answer->confirm_answer }}</td>

            <td>
                {!! Form::open(['route' => ['type-answers.destroy', $type_answer->id], 'method' =>
                'DELETE','class'=>'delete-item'.$type_answer->id]) !!}
                ...
                <button type="button" class="btn btn-md delete-item-table" data-toggle="tooltip"
                    data-id="{{$type_answer->id}}" title="Eliminar">
